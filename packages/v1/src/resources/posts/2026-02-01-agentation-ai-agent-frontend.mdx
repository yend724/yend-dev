...
[公式ドキュメント](https://agentation.dev/)でデモを試せる。以下は `YEND.DEV`（このブログ）でClaudeCodeとともに実際に使ってみた様子。
...
AIエージェントにUIの修正を伝える手段として、Agentationは便利そうだと感じた。ブラウザ上でセレクタを取得できるので、開発者がコードを追わなくてもエージェントに正確に伝えられるのが良い。しばらく使ってみようと思う。
