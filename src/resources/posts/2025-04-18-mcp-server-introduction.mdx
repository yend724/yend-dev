...
作成したMCPサーバーをCursor上で利用するために、以下の手順でMCPサーバーを登録する。
...
![CursorのMCPサーバーの設定画面で、uuid-generatorのMCPサーバーを設定した](/assets/posts/2025-04-18-mcp-server-introduction/mcp-servers-02.png)

## Cursorで実行している様子

ChatのAgentで「generate_uuid: UUIDを生成してください」のようにリクエストを投げると、`generate_uuid` ツールが呼び出され、UUIDが生成され、その結果が表示された。

![UUID生成のチャットリクエスト](/assets/posts/2025-04-18-mcp-server-introduction/uuid-generation-chat-request.png)
