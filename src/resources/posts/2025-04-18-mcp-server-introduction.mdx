...
<Image src="/assets/posts/2025-04-18-mcp-server-introduction/mcp-servers-02.png" alt="CursorのMCPサーバーの設定画面で、uuid-generatorのMCPサーバーを設定した" width={2406} height={464} />

## Cursorで実行している様子

ChatのAgentで「generate_uuid: UUIDを生成してください」のようにリクエストを投げると、`generate_uuid` ツールが呼び出され、UUIDが生成され、その結果が表示された。

<Image src="/assets/posts/2025-04-18-mcp-server-introduction/uuid-generation-chat-request.png" alt="UUID生成のチャットリクエスト" width={712} height={1012} />
