...
これらの式は、すべての項を左辺に移項することで、一般形`ay + bx + c = 0`という形に統一できます。ここで重要な点は:

- `a`と`b`の少なくとも一方は0以外の数である必要があります
- `a ≠ 0`のとき: `y = -(b/a)x - c/a`という形になります
- `a = 0`のとき: `x = -c/b`という形（x軸に平行な直線）になります
...
この実装により、一般形`ay + bx + c = 0`で表される直線を Canvas 上に描画することができました。